...
const ItemCard = ({ item }) => {
  const { estate_title,image_url, segment_name, price, status, area } = item || {};
  console.log();
  return (
    <div className="md:w-sm hover:shadow-sm hover:shadow-[#00AEFF] rounded-md shadow-md dark:bg-gray-50 dark:text-gray-800 border-2 cursor-pointer">
      ...
      <div>
        <p className="flex gap-1 items-center justify-center py-2">
          <FaLocationDot className="text-hero-color" /> Alberta
        </p>
      </div>
      <div className="text-center w-full">
        <NavLink className="w-full block border-y-2 border-[#0078B930] hover:bg-hero-color hover:text-white py-3 transition-all">
          VIEW DETAILS
        </NavLink>
      </div>
    </div>
